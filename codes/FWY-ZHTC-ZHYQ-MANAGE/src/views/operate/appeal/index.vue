    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="问题类型" prop="questionType">
        <el-select v-model="queryParams.questionType" placeholder="请选择问题类型" clearable>
          <el-option
            v-for="dict in dict.type.order_appeal_question_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
...
<!--      </el-form-item>-->
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['operate:appeal:add']"
        >发起申诉</el-button>
      </el-col>
    </el-row>

    <el-table v-loading="loading" :data="appealList" >
      ...
      <el-table-column label="问题类型" align="center" prop="questionType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.order_appeal_question_type" :value="scope.row.questionType"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="申诉时间" align="center" prop="createTime" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="180">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['operate:appeal:edit']"
          >查看</el-button>
          <el-button
            ...
            v-hasPermi="['operate:appeal:check']"
          >审核</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="问题类型" prop="questionType">
          <el-select :disabled="true" v-model="form.questionType" placeholder="请选择问题类型">
            <el-option
              v-for="dict in dict.type.order_appeal_question_type"
              :key="dict.value"
              :label="dict.label"
    ...
    <!-- 发起申诉 -->
    <el-dialog title="发起申诉" :visible.sync="openRefund" width="500px" append-to-body>
      <el-form ref="formRefund" :model="formRefund" :rules="rulesRefund" label-width="80px">
        <el-form-item label="问题类型" prop="questionType">
          <el-select v-model="formRefund.questionType" placeholder="请选择问题类型">
            <el-option
              v-for="dict in dict.type.order_appeal_question_type"
              :key="dict.value"
              :label="dict.label"
...
<script>
import { listAppeal, getAppeal, delAppeal, addAppeal, updateAppeal, checkOrderAppeal } from "@/api/operate/appeal";
import { listUser } from "@/api/operate/user";

export default {
  name: "Appeal",
  dicts: ['order_check_status', 'order_appeal_type', 'order_appeal_question_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const orderAppealId = row.orderAppealId || this.ids
      getAppeal(orderAppealId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.orderAppealId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const orderAppealIds = row.orderAppealId || this.ids;
      this.$modal.confirm('是否确认删除订单申诉编号为"' + orderAppealIds + '"的数据项？').then(function() {
        return delAppeal(orderAppealIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('operate/appeal/export', {
        ...this.queryParams
    ...
    submitFormRefund() {
      this.$refs["formRefund"].validate(valid => {
        if (valid) {
          addAppeal(this.formRefund).then(response => {
    ...
    submitFormCheck() {
      this.$refs["formCheck"].validate(valid => {
        if (valid) {
          if (this.formCheck.orderAppealId != null) {
            checkOrderAppeal(this.formCheck).then(response => {
